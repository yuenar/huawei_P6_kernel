...
//       GA=5000;
	...
	else if(E_APDS_TYPE_U9701G==apds_type)
	{
		if(COLOR_NV_BLACK==tp_color_for_edge)
		{
		GA=3000;
		if(cdata < 225*irdata/100)
			GA = GA*100/130;
			if(cdata >= 225*irdata/100&& cdata < 50*irdata/10)
				GA = GA*100/160;
			else
				GA = GA ;
...
static void apds990x_als_polling_work_handler(struct work_struct *work)
{
    ...
    if((E_APDS_TYPE_U9701L==apds_type)||(E_APDS_TYPE_U9701G==apds_type&&COLOR_NV_BLACK!=tp_color_for_edge)||(E_APDS_TYPE_U9700G==apds_type)||(E_APDS_TYPE_EDGE==apds_type))
	{
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (val == 1) {
		if(0==read_nv && (E_APDS_TYPE_EDGE_N==apds_type || E_APDS_TYPE_U9701G==apds_type))
		{
			tp_color_for_edge = read_tp_color_from_nv();
			printk("als:color=%d\n",tp_color_for_edge);
			read_nv=1;
			 if(E_APDS_TYPE_U9701G==apds_type)
			{
				if(COLOR_NV_BLACK==tp_color_for_edge)
				{
				   ...
				   const struct i2c_device_id *id)
{
	...
	{
		printk("the product is U9900, apds=%d\n", apds_type);
	...
	min_proximity_value = MAX_FAR_THRESHOLD;//MAX_ADC_PROX_VALUE - apds_990x_pwindows_value;
	printk("apds990x:pwave=%d, threshold=%d.\n", apds_990x_pwave_value, threshold_value);
